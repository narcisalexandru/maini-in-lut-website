<template>
  <div>
    ...
    <p v-if="!loading && !filteredLogs.length" class="text-center h-color-lunar-green py-8 m-0">
      {{ t("empty") }}
    </p>
  </div>
</template>
...
const { artistId, artist } = useArtistAdminContext();
...
const hasActiveFilters = computed(
  () => !!searchFilter.value.trim() || !!actionFilter.value,
);
...
const loadLogs = async () => {
  loading.value = true;
  try {
    const relatedLogs = await listAuditLogs({
      relatedArtistId: artistId.value,
      action: actionFilter.value || undefined,
    });

    const actorUserId = artist.value?.user?.id;
    const actorLogs = actorUserId
      ? await listAuditLogs({
          actorUserId,
          action: actionFilter.value || undefined,
        })
    ...
    logs.value = mergeLogs([...relatedLogs, ...actorLogs]);
  } catch (error) {
    toast.add({
      severity: "error",
      summary: t("error"),
...
<i18n lang="json">
{
  "ro": {
    "search": "Căutare",
    "searchPlaceholder": "Caută după nume, email, produs...",
    ...
    "target": "Țintă",
    "details": "Detalii",
    "empty": "Nicio acțiune înregistrată pentru acest artist.",
    "error": "Eroare",
  ...
  "en": {
    ...
    "target": "Target",
    "details": "Details",
    "empty": "No activity recorded for this artist.",
    "error": "Error",
    "roleSuperAdmin": "Super-admin",
    "roleArtist": "Artist",
    "reason": "Reason",

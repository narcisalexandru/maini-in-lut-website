<template>
  <div v-if="artist" class="space-y-6">
    <div class="h-bg-white rounded-xl shadow p-4 md:p-6 space-y-4">
      <h2 class="font-semibold h-color-lunar-green text-lg m-0">{{ t("applicant") }}</h2>
    ...
    <div class="h-bg-white rounded-xl shadow p-4 md:p-6 space-y-4">
      <h2 class="font-semibold h-color-lunar-green text-lg m-0">{{ t("application") }}</h2>
      <dl class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
        <div>
    ...
    >
      <h2 class="font-semibold h-color-lunar-green text-lg m-0">{{ t("accountManagement") }}</h2>
      <p class="text-sm h-color-lunar-green m-0">{{ accountManagementHint }}</p>
  ...
  >
    <div class="flex flex-col gap-2">
      <p class="text-sm h-color-lunar-green m-0">{{ t("suspendConfirm") }}</p>
  ...
  <Dialog
    ...
    :style="{ width: '480px' }"
  >
    <p class="text-sm h-color-lunar-green m-0">{{ t("deleteConfirm") }}</p>
    <template #footer>
...
const { artistId, artist, reloadArtist } = useArtistAdminContext();
const {
  approveArtist,
  rejectArtist,
  suspendArtist,
